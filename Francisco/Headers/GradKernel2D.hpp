...
double GradKernel2DX (double x, double y, double h)
{
	...
	if ( 1<=q && q<2)
		{
			R = (-3*x/(q*pow(h,2)))*pow(2-q,2)*(15/(14.0*pi*pow(h,2)));
		}
	if (2<=q)
		{
			R = 0;
...
double GradKernel2DY (double x, double y, double h)
{
	...
	if (q>=0 && q<1.0 )
		{	
			R = ((-3*y/(q*pow(h,2)))*pow((2-q),2)+(12*y/(q*pow(h,2)))*pow(1-q,2))*(15/(14.0*pi*pow(h,2)));
		}
	if ( 1<=q && q<2)
		{
			R = (-3*y/(q*pow(h,2)))*pow((2-q),2)*(15/(14.0*pi*pow(h,2)));
		}
	if (2<=q)
		{
			R = 0;
